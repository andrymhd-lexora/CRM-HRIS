...
export const HRISView: React.FC<HRISViewProps> = ({
  ...
  // RBAC: Hanya Admin, Super Admin, atau Owner yang berhak mengelola & melihat seluruh database karyawan.
  // Untuk user Staff dan Manager, data dibatasi hanya untuk profil karyawan milik user yang sedang login saja tanpa hak edit.
  const canManageEmployeeDatabase = !currentUser || currentUser?.role === 'Super Admin' || currentUser?.role === 'Owner' || currentUser?.role === 'Admin';
  const isAdminOrAbove = canManageEmployeeDatabase;
  ...
  const userEmp = React.useMemo(() => {
    ...
    return employees.find((e) => {
      ...
      if (userEmail && e.name) {
        const prefix = userEmail.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '');
        const cleanEmpName = e.name.toLowerCase().replace(/[^a-z0-9]/g, '');
        if (prefix && cleanEmpName && (cleanEmpName.includes(prefix) || prefix.includes(cleanEmpName))) return true;
      }
      return false;
    });
  ...
  const [empAvatarUrl, setEmpAvatarUrl] = useState<string>('');
  const [isUploadingPhoto, setIsUploadingPhoto] = useState(false);
  const empPhotoInputRef = React.useRef<HTMLInputElement | null>(null);
  const detailPhotoInputRef = React.useRef<HTMLInputElement | null>(null);

  // Photo compression & dataURL generator helper
  const handleEmployeePhotoFile = (file: File, onSuccess?: (dataUrl: string) => void) => {
    if (!file.type.startsWith('image/')) {
      alert('Format file harus berupa foto/gambar (JPG, PNG, WebP).');
      return;
    }
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        const maxDim = 400; // Optimal 400x400 for crisp & compact base64 storage
        if (width > height) {
          if (width > maxDim) {
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'employees' && (
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
              ...
              {canManageEmployeeDatabase && (
                <button
                  ...
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-md flex items-center gap-2 cursor-pointer transition-colors"
                >
                  <UserPlus className="w-4 h-4" />
          ...
          {/* Read-Only Notice & Access Info for Staff & Manager */}
          {!canManageEmployeeDatabase && (
            <div className="p-4 bg-gradient-to-r from-blue-50/90 to-indigo-50/90 border border-blue-200/90 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-extrabold text-xs text-blue-950">
                    Akses Data Karyawan Pribadi ({currentUser?.role || 'Staff / Manager'})
                  </h4>
                  <p className="text-[11px] text-blue-800 leading-relaxed">
                    Sesuai kebijakan akses, menu ini hanya menampilkan profil data Anda sendiri ({userEmp?.name || currentUser?.displayName || currentUser?.email}) dalam mode <strong>Lihat Data Saja (Read-Only)</strong>. Pengelolaan atau perubahan database karyawan hanya dapat dilakukan oleh <strong>Admin</strong> atau <strong>Owner</strong>.
                  </p>
                </div>
              </div>
              <span className="px-3 py-1 bg-white text-blue-700 text-[10px] font-extrabold rounded-lg border border-blue-200 shadow-2xs shrink-0 self-start sm:self-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredEmployees.length === 0 ? (
              <div className="col-span-full py-12 px-6 text-center bg-white rounded-2xl border border-slate-200/80 shadow-xs space-y-3">
                {!canManageEmployeeDatabase ? (
                  <>
                    ...
                    <h4 className="text-sm font-extrabold text-slate-800">
                      Profil Karyawan Belum Ditemukan
                    </h4>
                    <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                      Email akun login Anda (<strong className="font-bold text-slate-700">{currentUser?.email}</strong>) belum ditautkan dengan data karyawan di database HRIS. Silakan hubungi <strong>Admin HRD</strong> atau <strong>Owner</strong> untuk mendaftarkan profil Anda.
                    </p>
                  </>
                ) : (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative shrink-0">
                          <img
                            ...
                            alt={emp.name}
                            className="w-13 h-13 rounded-2xl object-cover border-2 border-slate-100 shadow-xs bg-slate-100"
                          />
                          {emp.avatarUrl && (
                            <span className="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-emerald-500 border-2 border-white rounded-full" title="Foto Terpasang" />
                          )}
                        </div>
                        <div>
        ...
        >
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-xl flex flex-col max-h-[88vh] overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="flex items-center gap-3.5">
                <div className="relative group shrink-0">
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${encodeURIComponent(viewingEmployee.name)}&background=3B82F6&color=fff`
                    }
                    alt={viewingEmployee.name}
                    className="w-13 h-13 sm:w-14 sm:h-14 rounded-2xl object-cover border-2 border-slate-700 shadow-md bg-slate-800"
                  />
                  {canManageEmployeeDatabase && (
                    <button
                      type="button"
                    ...
                    >
                      <Camera className="w-4 h-4" />
                      <span className="text-[8px] font-bold mt-0.5">Ganti Foto</span>
                    </button>
                  )}
                  <input
                    type="file"
                    ...
                    onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (file && viewingEmployee?.id) {
                        handleEmployeePhotoFile(file, async (dataUrl) => {
                          await onUpdateEmployee(viewingEmployee.id, { avatarUrl: dataUrl });
                          setViewingEmployee((prev) => (prev ? { ...prev, avatarUrl: dataUrl } : null));
                          showToast('Foto profil karyawan berhasil diperbarui!');
                        });
                      }
                    }}
                    className="hidden"
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[10px] font-mono text-blue-400 font-bold">NIK: {viewingEmployee.employeeCode}</span>
                    {viewingEmployee.avatarUrl && (
                      <span className="text-[9px] font-bold text-emerald-400 bg-emerald-950/60 border border-emerald-800 px-1.5 py-0.2 rounded-md">
                        ✓ Foto Tersedia
                      </span>
                    )}
                  </div>
            ...
            <div className="p-3.5 sm:p-4 border-t border-slate-100 bg-slate-50/80 flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-2">
                {!canManageEmployeeDatabase && (
                  <span className="px-3 py-1.5 bg-blue-50 text-blue-700 text-[11px] font-bold rounded-xl border border-blue-200">
                    Mode Lihat Profil (Read-Only)
                  </span>
                )}

                {canManageEmployeeDatabase && viewingEmployee?.id && (
                  <button
                    type="button"
                    onClick={() => {
                ...
                {canManageEmployeeDatabase && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setEmpAvatarUrl(viewingEmployee.avatarUrl || '');
                      setViewingEmployee(null);
                      setIsAddEmpModalOpen(true);
                    }}
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-3xl flex flex-col max-h-[90vh] overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
            ...
            <form
              onSubmit={async (e) => {
                ...
                const data: Partial<Employee> = {
                  ...
                  avatarUrl: empAvatarUrl || ''
                };

                if (editingEmployee && editingEmployee.id) {
                  await onUpdateEmployee(editingEmployee.id, data);
                  showToast('Data & foto karyawan berhasil diperbarui!');
                } else {
            ...
            >
              <div className="p-4 sm:p-5 overflow-y-auto space-y-4 text-xs font-medium flex-1">
                {/* Section Foto Karyawan */}
                <div className="p-4 bg-gradient-to-r from-blue-50/80 to-indigo-50/80 rounded-2xl border border-blue-200/80 flex flex-col sm:flex-row items-center gap-4">
                  <div className="relative group shrink-0">
                    <img
                      src={
                        empAvatarUrl ||
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(editingEmployee?.name || 'Karyawan')}&background=3B82F6&color=fff`
                      }
                      alt="Foto Karyawan"
                      className="w-20 h-20 sm:w-22 sm:h-22 rounded-2xl object-cover border-2 border-white shadow-md bg-white"
                    />
                    {empAvatarUrl && (
                      <button
                        type="button"
                        onClick={() => setEmpAvatarUrl('')}
                        className="absolute -top-1.5 -right-1.5 p-1 bg-rose-600 hover:bg-rose-700 text-white rounded-full shadow cursor-pointer transition-transform hover:scale-110"
                        title="Hapus Foto"
                      >
                        <X className="w-3.5 h-3.5" />
                      </button>
                  ...
                  <div className="space-y-1.5 text-center sm:text-left flex-1">
                    <div className="flex items-center justify-center sm:justify-start gap-2">
                      <h4 className="text-xs font-extrabold text-slate-900">Upload Foto Profil Karyawan</h4>
                      <span className="px-2 py-0.5 rounded-full text-[9px] font-extrabold bg-blue-100 text-blue-800 border border-blue-200">
                        Muncul di Database & Profil
                      </span>
                    </div>
                    <p className="text-[11px] text-slate-600 leading-snug">
                      Format didukung: JPG, PNG, WebP. Foto akan otomatis dioptimasi dan ditampilkan pada icon kartu pegawai dan seluruh profil HRIS.
                    </p>
                    ...
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 pt-1">
                      <input
                        ...
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) handleEmployeePhotoFile(file);
                        }}
                        className="hidden"
                      />
                      <button
                        type="button"
                        onClick={() => empPhotoInputRef.current?.click()}
                        disabled={isUploadingPhoto}
                        className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white font-bold text-xs rounded-xl shadow-xs flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                        <Upload className="w-3.5 h-3.5" />
                        <span>{empAvatarUrl ? 'Ganti Foto' : 'Pilih Foto Karyawan'}</span>
                      </button>
                      {empAvatarUrl && (
                        <button
                          type="button"
                          onClick={() => setEmpAvatarUrl('')}
                          className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-bold rounded-xl border border-rose-200 flex items-center gap-1 transition-colors cursor-pointer"
                        >
              ...
              <div className="p-3.5 sm:p-4 border-t border-slate-100 bg-slate-50/80 flex items-center justify-between gap-2 shrink-0">
                {editingEmployee && editingEmployee.id && canManageEmployeeDatabase ? (
                  <button
                    type="button"
                    onClick={() => {
